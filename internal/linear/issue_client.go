...
func (ic *IssueClient) CreateIssue(title, description, teamID string) (*Issue, error) {
	...
	if title == "" {
		return nil, ValidationErrorWithExample("title", "cannot be empty",
			`linear_create_issue("Fix login bug", "Bug in authentication flow", "team-123")`)
	}
	if teamID == "" {
		return nil, ValidationErrorWithExample("teamID", "cannot be empty",
...
linear_create_issue("Task title", "Description", teams[0].id)`)
	...
	if response.IssueCreate.Issue.Description != "" {
		metadata, cleanDesc := extractMetadataFromDescription(response.IssueCreate.Issue.Description)
...
func (ic *IssueClient) GetIssue(issueID string) (*Issue, error) {
	...
	if issueID == "" {
		return nil, &ValidationError{Field: "issueID", Message: "issueID cannot be empty"}
	}
	...
	if response.Issue.Description != "" {
		metadata, cleanDesc := extractMetadataFromDescription(response.Issue.Description)
...
func (ic *IssueClient) getIssueWithProjectContextInternal(issueID string) (*Issue, error) {
	if issueID == "" {
		return nil, &ValidationError{Field: "issueID", Message: "issueID cannot be empty"}
	}
	...
	if response.Issue.Project != nil && response.Issue.Project.Description != "" {
		projectMetadata, cleanProjectDesc := extractMetadataFromDescription(response.Issue.Project.Description)
...
func (ic *IssueClient) getIssueWithParentContextInternal(issueID string) (*Issue, error) {
	if issueID == "" {
		return nil, &ValidationError{Field: "issueID", Message: "issueID cannot be empty"}
	}
	...
	if response.Issue.Parent != nil && response.Issue.Parent.Description != "" {
		parentMetadata, cleanParentDesc := extractMetadataFromDescription(response.Issue.Parent.Description)
...
func (ic *IssueClient) UpdateIssueState(issueID, stateID string) error {
	...
	if issueID == "" {
		return &ValidationError{Field: "issueID", Message: "issueID cannot be empty"}
	}
	if stateID == "" {
		return &ValidationError{Field: "stateID", Message: "stateID cannot be empty"}
	}
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "Entity not found in validateAccess: stateId") || 
		   strings.Contains(err.Error(), "does not exist") && strings.Contains(err.Error(), "state") {
			return InvalidStateIDError(stateID, err)
		}
		return EnhanceGenericError("update issue state", err)
	}
	
	if !response.IssueUpdate.Success {
		return OperationFailedError("Update issue state", "issue", []string{
...
func (ic *IssueClient) ListAssignedIssues(limit int) ([]Issue, error) {
	...
	for i := range response.Issues.Nodes {
		if response.Issues.Nodes[i].Description != "" {
			metadata, cleanDesc := extractMetadataFromDescription(response.Issues.Nodes[i].Description)
...
func (ic *IssueClient) BatchUpdateIssues(issueIDs []string, update BatchIssueUpdate) (*BatchIssueUpdateResult, error) {
	...
	var response struct {
		IssueBatchUpdate BatchIssueUpdateResult `json:"issueBatchUpdate"`
	}
...
func (ic *IssueClient) GetSubIssues(parentIssueID string) ([]SubIssue, error) {
	if parentIssueID == "" {
		return nil, &ValidationError{Field: "parentIssueID", Message: "parentIssueID cannot be empty"}
	}
...
func (ic *IssueClient) UpdateIssueDescription(issueID, newDescription string) error {
	...
	if issue.Metadata != nil && len(issue.Metadata) > 0 {
		descriptionWithMetadata = injectMetadataIntoDescription(newDescription, issue.Metadata)
	}
...
func (ic *IssueClient) UpdateIssueMetadataKey(issueID, key string, value interface{}) error {
	...
	if !isValidMetadataKey(key) {
		return &ValidationError{Field: "key", Value: key, Reason: "must be alphanumeric with underscores or hyphens, starting with letter or underscore"}
	}
	...
	descriptionWithMetadata := injectMetadataIntoDescription(issue.Description, issue.Metadata)
...
func (ic *IssueClient) GetIssueSimplified(issueID string) (*Issue, error) {
	...
	response.Issue.Children.Nodes = []SubIssue{}
...
func (ic *IssueClient) UpdateIssue(issueID string, input UpdateIssueInput) (*Issue, error) {
	...
	if !hasFieldsToUpdate(input) {
		return nil, &ValidationError{Field: "input", Message: "no fields to update"}
	}
	
	// Validate priority if provided
	if input.Priority != nil && (*input.Priority < 0 || *input.Priority > 4) {
		return nil, &ValidationError{Field: "priority", Message: fmt.Sprintf("invalid priority value: %d (must be between 0-4)", *input.Priority)}
	}
	...
	if input.Description != nil {
		...
		if issue.Metadata != nil && len(issue.Metadata) > 0 {
			descWithMetadata := injectMetadataIntoDescription(*input.Description, issue.Metadata)
	...
	if response.IssueUpdate.Issue.Description != "" {
		metadata, cleanDesc := extractMetadataFromDescription(response.IssueUpdate.Issue.Description)
...
func hasFieldsToUpdate(input UpdateIssueInput) bool {
...
func buildUpdateInput(input UpdateIssueInput) map[string]interface{} {
...
func (ic *IssueClient) ListAllIssues(filter *IssueFilter) (*ListAllIssuesResult, error) {
	...
	if filter == nil {
		return nil, &ValidationError{Field: "filter", Message: "filter cannot be nil"}
	}
	if filter.First <= 0 {
	...
	for _, node := range response.Issues.Nodes {
		...
		if issue.Description != "" {
			metadata, cleanDesc := extractMetadataFromDescription(issue.Description)
			if len(metadata) > 0 {
		...
		if issue.Project != nil && issue.Project.Description != "" {
			projectMetadata, cleanProjectDesc := extractMetadataFromDescription(issue.Project.Description)
...
func hasFilters(filter *IssueFilter) bool {
...
func buildFilterObject(filter *IssueFilter) map[string]interface{} {
...
func (ic *IssueClient) ListIssueAttachments(issueID string) ([]Attachment, error) {
	// Validate input
	if issueID == "" {
		return nil, ValidationErrorWithExample("issueID", "cannot be empty", 
...
func (ic *IssueClient) GetIssueWithRelations(issueID string) (*IssueWithRelations, error) {
	...
	var response struct {
		Issue IssueWithRelations `json:"issue"`
	}
...
func (ic *IssueClient) GetTeamIssuesWithRelations(teamID string, limit int) ([]IssueWithRelations, error) {
	...
	var response struct {
		Issues struct {
			Nodes []IssueWithRelations `json:"nodes"`
		} `json:"issues"`
	}
